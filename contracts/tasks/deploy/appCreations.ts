...
task("deploy:AppCreations").setAction(async function (_taskArguments: TaskArguments, { ethers }) {
  ...
  const appCreations: AppCreations = <AppCreations>await appCreationsFactory.connect(deployer).deploy();
